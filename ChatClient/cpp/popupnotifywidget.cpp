...
PopupNotifyWidget::PopupNotifyWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(&animation, &QAbstractAnimation::finished, this, &PopupNotifyWidget::hide); /* Подключаем сигнал окончания                                                                  * */
    ...
    timer = new QTimer();
    connect(timer, &QTimer::timeout, this, &PopupNotifyWidget::hideAnimation);
}

void PopupNotifyWidget::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
void PopupNotifyWidget::show()
{
    ...
    setGeometry(_screen->availableGeometry().width() - 36 - width() + _screen->availableGeometry().x(),
                _screen->availableGeometry().height() - 36 - height() + _screen->availableGeometry().y(),
                width(),
                height());
...
void PopupNotifyWidget::hideAnimation()
{
...
void PopupNotifyWidget::setPopupOpacity(float opacity)
{
...
float PopupNotifyWidget::getPopupOpacity() const
{
    return popupOpacity;
}
